"""
╔══════════════════════════════════════════════════════════════╗
║      𝙕𝙔 𝘾𝙤𝙢𝙗𝙤 𝙎𝙘𝙖𝙣𝙣𝙚𝙧 𝘽𝙤𝙩  ·  𝙑𝟖.𝟎         ║
╚══════════════════════════════════════════════════════════════╝

Features:
  • Free tier  — 1 link at a time, 10-part split, 500 MB cap (default)
  • VIP tier   — unlimited links, 20-part split, no cap, max speed
  • GOD MODE   — 25-part split, 15 concurrent workers, maximum speed
  • Plan-based split download engine:
      FREE → 10 parts (4 active)  VIP → 20 parts (10 active)
      GOD  → 25 parts (15 active)
  ...
  • Per-part retry (3×) with exponential backoff + jitter
...
# ─────────────────────────────────────────────────────────────────────
#  Config - GOD MODE UPGRADED TO 35x THREADS
# ─────────────────────────────────────────────────────────────────────
...
CHUNK_SIZE_VIP   = 1024 * 1024 * 2   # 2 MB — faster reads for VIP
CHUNK_SIZE_GOD   = 1024 * 1024 * 4   # 4 MB — God Mode chunk
...
N_PARTS_VIP  = 20   # VIP   → 20 parts
N_PARTS_GOD  = 25   # GOD   → 25 parts

# Max concurrent parts active at the same time (semaphore workers)
MAX_WORKERS_FREE = 4   # 10 parts, 4 active at once
MAX_WORKERS_VIP  = 10  # 20 parts, 10 active at once
MAX_WORKERS_GOD  = 15  # 25 parts, 15 active at once
...
MAX_RETRIES      = 3
MAX_RETRIES_GOD  = 10               # GOD MODE: More retries for maximum reliability
PROGRESS_STEP    = 5               # % step for live bar update

# Timeout settings (seconds)
CONNECT_TIMEOUT_FREE = 60
CONNECT_TIMEOUT_VIP  = 30
CONNECT_TIMEOUT_GOD  = 15           # Faster timeout, more retries for God Mode

SOCK_READ_TIMEOUT_FREE = 300
SOCK_READ_TIMEOUT_VIP  = 180
SOCK_READ_TIMEOUT_GOD  = 120        # Shorter reads, faster recovery

# Exponential backoff settings
BASE_RETRY_DELAY = 1.0              # Base delay in seconds
MAX_RETRY_DELAY = 30.0              # Maximum delay cap
JITTER_RANGE = 0.5                  # Random jitter factor (0-50%)
...
def _strip_url(line: str) -> str:
    ...
    # ── Pipe-separated: url|email:pass or url|user:pass ──────────────
    if '|' in line:
        parts = line.split('|')
        cred_parts = [p for p in parts[1:] if p]
        if cred_parts:
            return '|'.join(cred_parts).strip()

    # ── email:pass — first segment has @ sign, keep whole line ───────
    # e.g. user@gmail.com:password123  →  user@gmail.com:password123
    colon_idx = line.find(':')
    if colon_idx > 0 and '@' in line[:colon_idx]:
        return line  # preserve email:pass as-is

    # ── URL-prefixed: strip only the http(s)://domain/path part ──────
    # e.g. https://site.com:user@gmail.com:pass  or  site.com:email:pass
    if re.match(r'^https?://', line, re.IGNORECASE):
        stripped = re.sub(r'^https?://[^:]+', '', line, flags=re.IGNORECASE).lstrip(':').strip()
        if stripped:
            # If what remains starts with an email, return it whole
    ...
    # ── Legacy colon-split: skip domain portion ───────────────────────
    parts = line.split(':')
    if len(parts) < 2:
        return line
    split_at = None
    ...
    if split_at is None:
        split_at = 1
    return ':'.join(parts[split_at:]).strip()
...
async def _process_link_streaming(session, url: str, idx: int, job: Job) -> tuple[dict, Optional[Path]]:
    ...
    base_headers = {"User-Agent": "Mozilla/5.0 (compatible; ZyScanBot/7.0)"}
...
async def run_job(job: Job):
    ...
    # GOD MODE: Enhanced connection pool for 50 parallel connections
    if job.god:
        connector = aiohttp.TCPConnector(
            limit=70,               # Total connection pool
            limit_per_host=35,      # Per-host limit
            ttl_dns_cache=300,
            force_close=False,
...
def main():
    ...
    log.info("Starting ZY Combo Scanner Bot v8 — plan-split streaming engine…")
