...
def resolve_linkforge(url: str) -> Optional[str]:
    ...
    These typically return a Location header or serve the file directly.
    """
    scraper = cloudscraper.create_scraper()
    try:
        # Follow redirects manually so we can capture each hop
        r = scraper.get(url, allow_redirects=True, timeout=20,
                        headers={"User-Agent": "Mozilla/5.0"})
        ct  = r.headers.get("Content-Type", "")
        cd  = r.headers.get("Content-Disposition", "")
        # Final URL is a direct file
        if "text/html" not in ct or cd:
            return r.url
        # Fallback: look for a direct download link in the page
        soup = BeautifulSoup(r.text, "html.parser")
        for a in soup.find_all("a", href=True):
            href = a["href"].strip()
            if any(href.lower().endswith(ext) for ext in (
                ".zip", ".rar", ".7z", ".tar.gz", ".gz", ".pdf",
        ...
        # Last resort: return the final URL as-is and let the downloader try
        return r.url
    except Exception as e:
...
def resolve_flowbit(url: str) -> Optional[str]:
    ...
    try:
        headers = {
            ...
            "Referer": url,  # Some flowbit endpoints check Referer
        }
        r = scraper.get(url, allow_redirects=True, timeout=25, headers=headers)
        ct  = r.headers.get("Content-Type", "")
        cd  = r.headers.get("Content-Disposition", "")
        if "text/html" not in ct or cd:
            return r.url
        # If we got HTML, check for a direct link inside
        soup = BeautifulSoup(r.text, "html.parser")
        for a in soup.find_all("a", href=True):
            href = a["href"].strip()
            if "download" in href.lower() or "dl" in href.lower():
                return href if href.startswith("http") else urljoin(r.url, href)
        # Return final URL and let downloader attempt it
        return r.url
    except Exception as e:
...
def smart_resolve(url: str) -> dict:
    ...
    try:
        ...
        if "flowbit" in host and "herokuapp.com" in host:
            ...
            result["direct_url"] = direct
            if not direct:
                result["error"] = "Could not resolve Flowbit link."
            return result
...
def current_speed_cap() -> int:
    """Return the active speed cap in bytes/sec. No slow-hours throttle."""
...
KW_FETCH_WORKERS = 20
KW_FETCH_TIMEOUT = aiohttp.ClientTimeout(total=20, connect=8)
KW_MAX_PAGES     = 500
KW_FUZZY_RATIO   = 0.85
...
async def _fetch_page(session: aiohttp.ClientSession, url: str, semaphore: asyncio.Semaphore) -> str:
    async with semaphore:
        for attempt in range(1, 3):
            try:
                async with session.get(
                    ...
                    headers={"User-Agent": "Mozilla/5.0 (compatible; TGBot/5.0)"},
                ) as r:
                    if r.status == 200:
                        ct = r.headers.get("Content-Type", "")
...
async def keyword_extract(url: str, keywords: list[str], status_msg, deep: bool = True) -> list[str]:
    kw_lower = [k.lower().strip() for k in keywords if k.strip()]
    semaphore = asyncio.Semaphore(KW_FETCH_WORKERS)
    connector = aiohttp.TCPConnector(limit=KW_FETCH_WORKERS + 4, ttl_dns_cache=300)
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        seed_html = await _fetch_page(session, url, semaphore)
        if not seed_html:
            return []

        all_urls = [url]
        if deep:
            all_urls += _discover_subpages(url, seed_html, KW_MAX_PAGES - 1)

        await status_msg.edit_text(
            f"🔍 *Scanning {len(all_urls)} page(s)…*\n"
            f"Keywords: `{', '.join(kw_lower)}`",
            parse_mode=ParseMode.MARKDOWN,
        )

        tasks = [_fetch_page(session, u, semaphore) for u in all_urls[1:]]
        pages_html = [seed_html] + list(await asyncio.gather(*tasks))
    ...
    await status_msg.edit_text(
        f"🧹 *Deduplicating {len(all_matches)} matches…*",
        parse_mode=ParseMode.MARKDOWN,
    )
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, deduplicate, all_matches)
...
async def keyword_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🔍 *Keyword Extractor*\n\n"
        "Step 1️⃣ — Send the *URL* you want to search:\n\n"
        "_I'll scan the page (and sub-pages) for lines containing your keywords._\n\n"
        "/cancel to stop",
        parse_mode=ParseMode.MARKDOWN,
    )
    return KW_WAITING_URL


async def kw_receive_url(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.strip()
    match = URL_RE.search(text)
    if not match:
        await update.message.reply_text("❌ No valid URL detected. Please send a URL, or /cancel.")
        return KW_WAITING_URL
    ctx.user_data["kw_url"] = match.group(0)
    await update.message.reply_text(
        f"✅ URL saved: `{ctx.user_data['kw_url']}`\n\n"
        "Step 2️⃣ — Send your *keyword(s)* (comma or newline separated):\n"
        "_e.g._ `price, discount, sale`\n\n"
        "/cancel to stop",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def kw_receive_keywords(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not keywords:
        await update.message.reply_text("❌ No keywords found. Try again or /cancel.")
        return KW_WAITING_KEYWORD
    ...
    try:
        results = await keyword_extract(url, keywords, status, deep=True)

        if not results:
            await status.edit_text(
                f"🔍 *No matches found.*\n\n"
                f"Keywords `{', '.join(keywords)}` were not found on that page.",
                parse_mode=ParseMode.MARKDOWN,
            )
            ctx.user_data.clear()
            return ConversationHandler.END
        ...
        header = (
            ...
            f"URL: {url}\n"
            f"Keywords: {', '.join(keywords)}\n"
            f"Total unique matches: {len(results)}\n"
            f"{'=' * 60}\n\n"
        )
        body = "\n".join(results)
        filename = f"keywords_{int(time.time())}.txt"
        ...
        logger.exception("keyword_extract error")
...
def main():
    ...
    # Keyword extractor conversation
    kw_conv = ConversationHandler(
        entry_points=[CommandHandler("keyword", keyword_cmd)],
        states={
            KW_WAITING_URL:     [MessageHandler(filters.TEXT & ~filters.COMMAND, kw_receive_url)],
