...
OWNER_ID         = int(os.environ.get("OWNER_ID", "5028065177"))
...
def current_speed_cap() -> int:
    """Return the current speed cap in bytes per second.
    Can be extended to support time-based scheduling in the future.
    """
    return MAX_SPEED_BPS
# ─────────────────────────────────────────────────────────────
...
def resolve_flowbit(url: str) -> Optional[str]:
    ...
    _path_decoded = _uq(_up(url).path)
    _ext = Path(_path_decoded).suffix.lower()
    ...
    for hdrs in headers_variants:
        try:
            r = sess.get(url, allow_redirects=True, timeout=30, headers=hdrs, stream=True)
            ct = r.headers.get("Content-Type", "").lower()
            cd = r.headers.get("Content-Disposition", "")

            if r.status_code in (401, 403, 404, 410):
                logger.warning(f"Flowbit: HTTP {r.status_code} — link expired or invalid code")
                r.close()
                return None

            if "text/html" not in ct or cd:
                logger.info(f"Flowbit: direct file ct={ct!r} ua={hdrs['User-Agent'][:25]}")
                final_url = str(r.url)
                r.close()
                return final_url
            ...
            if any(phrase in body.lower() for phrase in (
                "link has expired", "invalid code", "download expired",
                "token invalid", "link expired", "not found", "404",
            )):
                logger.warning("Flowbit: expired/invalid link detected in HTML body")
                return None
            ...
            for a in soup.find_all("a", href=True):
                href = a["href"].strip()
                if (
                    any(kw in href.lower() for kw in ("/dl/", "?code=", "download", "file="))
                    or (_ext and href.lower().endswith(_ext))
                ):
                    resolved = href if href.startswith("http") else urljoin(url, href)
                    logger.info(f"Flowbit: link found in HTML: {resolved}")
                    return resolved
            ...
            if meta and meta.get("content"):
                ...
                if m:
                    ...
                    if not loc.startswith("http"):
                        loc = urljoin(url, loc)
                    logger.info(f"Flowbit: meta-refresh → {loc}")
                    return loc
            ...
            for script in soup.find_all("script"):
                ...
                if js_m:
                    ...
                    if not loc.startswith("http"):
                        loc = urljoin(url, loc)
                    logger.info(f"Flowbit: JS redirect → {loc}")
                    return loc

                if _ext:
                    url_in_js = re.search(
                        r"""['"]( ?https?://[^'"]+""" + re.escape(_ext) + r"""[^'"]*)['"]""",
                        text, re.I,
                    )
                    if url_in_js:
                        logger.info(f"Flowbit: URL in JS → {url_in_js.group(1)}")
                        return url_in_js.group(1)

            logger.warning(f"Flowbit: HTML returned, no link found ua={hdrs['User-Agent'][:30]!r}")

        except Exception as exc:
            logger.warning(f"Flowbit attempt failed ua={hdrs.get('User-Agent','?')[:25]}: {exc}")
            continue
    ...
    try:
        ...
        r = scraper.get(
            ...
            headers={"Accept": "text/plain, application/octet-stream, */*", "Referer": url},
        )
        ct = r.headers.get("Content-Type", "").lower()
        cd = r.headers.get("Content-Disposition", "")
        if "text/html" not in ct or cd:
            logger.info(f"Flowbit: cloudscraper fallback got file ct={ct!r}")
            return r.url
    except Exception as exc:
        logger.warning(f"Flowbit cloudscraper fallback failed: {exc}")

    logger.warning("Flowbit: ALL strategies returned HTML — link may be expired")
    return None
...
def resolve_generic_page(url: str) -> Optional[str]:
    ...
    try:
        ...
        ext_re = re.compile(
            r"\.(txt|zip|rar|7z|tar\.gz|gz|bz2|xz|pdf|mp4|mkv|avi|mov|mp3|"
            r"flac|wav|exe|apk|iso|dmg|docx?|xlsx?|pptx?|torrent|bin|img|csv|log)\b",
            re.I,
        )
        for a in soup.find_all("a", href=True):
            href = a["href"].strip()
...
def smart_resolve(url: str) -> dict:
    ...
    # ── Fast-path: URL path already ends with a known downloadable extension ──
    # e.g. flowbit /dl/.../file.txt?code=... → skip all scraping, go direct
    _DIRECT_EXTS = {
        ".txt", ".zip", ".rar", ".7z", ".gz", ".bz2", ".xz",
        ".pdf", ".mp4", ".mkv", ".avi", ".mov", ".mp3", ".flac", ".wav",
        ".exe", ".apk", ".iso", ".dmg", ".docx", ".xlsx", ".pptx",
        ".torrent", ".bin", ".img", ".csv", ".json", ".log",
    }
    _path_decoded = unquote(parsed.path)
    _url_ext = Path(_path_decoded).suffix.lower()
    if _url_ext in _DIRECT_EXTS and not ("flowbit" in host and "herokuapp.com" in host):
        # Probe with plain requests first (no browser fingerprint)
        _probe_headers = [
            {"User-Agent": "python-requests/2.31.0", "Accept": "text/plain, application/octet-stream, */*"},
            {"User-Agent": "aria2/1.36.0", "Accept": "*/*"},
            {"User-Agent": "Wget/1.21.3", "Accept": "*/*", "Referer": url},
        ]
        for _ph in _probe_headers:
            try:
                _pr = requests.head(url, allow_redirects=True, timeout=15, headers=_ph)
                _ct = _pr.headers.get("Content-Type", "").lower()
                _cd = _pr.headers.get("Content-Disposition", "")
                if "text/html" not in _ct or _cd:
                    logger.info(f"smart_resolve: direct-ext fast-path → {_pr.url}")
                    result["direct_url"] = _pr.url
                    if "filename=" in _cd:
                        result["filename_hint"] = _cd.split("filename=")[-1].strip().strip('"\'')
                    return result
            except Exception as _e:
                logger.debug(f"smart_resolve fast-path probe failed: {_e}")
                continue
        logger.info(f"smart_resolve: direct-ext fast-path probes returned HTML, falling through to resolver")
    ...
    try:
        ...
        if "flowbit" in host and "herokuapp.com" in host:
            # If URL already has ?code= and a file extension, probe it first.
            # Flowbit servers sometimes return HTML even for direct ?code= URLs
            # (expired token, Cloudflare challenge, etc.), so we must verify
            # the response is actually a file before trusting it.
            if "code=" in parsed.query and _url_ext in _DIRECT_EXTS:
                _flowbit_probe_headers = [
                    {
                        "User-Agent": "python-requests/2.31.0",
                        "Accept": "text/plain, application/octet-stream, */*",
                    },
                ...
                _probe_ok = False
                for _ph in _flowbit_probe_headers:
                    try:
                        _pr = requests.head(url, allow_redirects=True, timeout=15, headers=_ph)
                        _pct = _pr.headers.get("Content-Type", "").lower()
                        _pcd = _pr.headers.get("Content-Disposition", "")
                        if "text/html" not in _pct or _pcd:
                            logger.info(f"Flowbit: direct code URL probe OK → {_pr.url}")
                            result["direct_url"] = str(_pr.url)
                            result["filename_hint"] = (
                                _pcd.split("filename=")[-1].strip().strip('"\'')
                                if "filename=" in _pcd
                                else unquote(Path(parsed.path).name)
                            )
                            _probe_ok = True
                            break
                    except Exception as _pe:
                        logger.debug(f"Flowbit direct probe failed: {_pe}")
                        continue
                if _probe_ok:
                    return result
                # Probe returned HTML — fall through to resolver
                logger.info("Flowbit: direct code URL probe returned HTML, falling through to resolver")
...
async def parallel_download(
    ...
    # Use a plain downloader UA — browser-like UAs cause some servers (e.g. Flowbit/Heroku)
    # to return an HTML download-page instead of the raw file.
    req_headers.setdefault("User-Agent", "python-requests/2.31.0")
    req_headers.setdefault("Accept", "text/plain, application/octet-stream, */*")
    ...
    async with aiohttp.ClientSession(connector=connector, headers=req_headers) as session:
        async with session.head(url, allow_redirects=True, timeout=CONNECT_TIMEOUT) as head:
            _head_ct = head.headers.get("Content-Type", "").lower()
            _head_cd = head.headers.get("Content-Disposition", "")
            # Guard: if HEAD returns HTML with no Content-Disposition, the link
            # is a download/error page — abort before wasting bandwidth.
            if "text/html" in _head_ct and not _head_cd:
                raise ValueError(
                    "\u26a0\ufe0f The server returned an HTML page instead of the actual file.\n\n"
                    "This usually means the download link has *expired* or the ?code= token "
                    "is no longer valid.\n"
                    "Please generate a fresh download link and try again."
                )
...
async def try_ytdlp(url: str, update: Update, status_msg) -> bool:
    ...
    try:
        ...
        for f in files:
            size = f.stat().st_size
            is_valid, type_error = _validate_file_type(f)
            if not is_valid:
                f.unlink(missing_ok=True)
                await update.message.reply_text(type_error, parse_mode=ParseMode.MARKDOWN)
                continue
            await status_msg.edit_text(
...
async def handle_password(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await do_download(direct, update, ctx, status, filename_hint=info.get("filename_hint"))
    return ConversationHandler.END
...
def validate_bot_token(token: str) -> bool:
    ...
    if not token:
        return False
    # Telegram bot tokens follow format: NUMBER:ALPHANUMERIC
...
def main():
    ...
    if not validate_bot_token(BOT_TOKEN):
        logger.warning(
            "⚠️  BOT_TOKEN format looks invalid. "
    ...
    logger.info(f"⚡ Smart Download Bot v6.1 — {PARALLEL_WORKERS} parallel workers — starting…")
