...
def resolve_flowbit(url: str) -> Optional[str]:
    ...
    Root cause of old bug: cloudscraper mimics a real browser → the Heroku server
    detects it and returns an HTML download-page instead of the raw file.
    Fix: try plain requests (no browser fingerprint) first, then fall back to scraper.
    The ?code= query param IS the auth token — it must be preserved in every request.
    """
    # ── Detect expected file extension from the URL path ──────────────────
    from urllib.parse import urlparse as _up, unquote as _uq
    _path_decoded = _uq(_up(url).path)
    _ext = Path(_path_decoded).suffix.lower()  # e.g. ".txt", ".zip"

    # Header sets to try — plain requests first, then progressively more browser-like
    headers_variants = [
        # 1. Minimal — looks like wget/curl: most likely to get the raw file
        {
        ...
        # 3. wget-style with Referer
        {
            "User-Agent": "Wget/1.21.3",
            "Accept": "*/*",
            "Referer": url,
        },
        # 4. Lightweight browser (no heavy fingerprint)
        {
    ...
    for hdrs in headers_variants:
        try:
            ...
            # ── Not HTML (text/plain, application/octet-stream, etc.) → direct file ──
            if "text/html" not in ct or cd:
                logger.info(f"Flowbit: direct file (ct={ct!r}, cd={cd!r}) ua={hdrs['User-Agent'][:25]}")
                final_url = str(r.url)
                r.close()
                return final_url

            # ── Got HTML — read body and mine it for the real download URL ──
            body = r.text
            r.close()
            soup = BeautifulSoup(body, "html.parser")

            # a) <a href> containing the expected extension or download keywords
            for a in soup.find_all("a", href=True):
                href = a["href"].strip()
                if (
            ...
            # c) JS window.location / location.href / location.replace
            for script in soup.find_all("script"):
                ...
                # d) any URL in JS that ends with expected file extension
                if _ext:
                    url_in_js = re.search(
            ...
            logger.warning(f"Flowbit attempt failed ({hdrs.get('User-Agent','?')[:25]}): {exc}")
            continue

    # ── Final fallback: cloudscraper (heavy browser emulation) ────────────
    try:
        scraper = cloudscraper.create_scraper()
        r = scraper.get(
    ...
    if _url_ext in _DIRECT_EXTS:
        ...
        for _ph in _probe_headers:
            try:
                ...
                continue
        # Fast-path probes all got HTML — fall through to per-host resolvers below
