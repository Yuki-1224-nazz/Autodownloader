...
def smart_resolve(url: str) -> dict:
    ...
    try:
        ...
        if "flowbit" in host and "herokuapp.com" in host:
            # If URL already has ?code= and a file extension, it IS the direct link.
            # Don't probe/resolve — pass straight to the downloader.
            if "code=" in parsed.query and _url_ext in _DIRECT_EXTS:
                logger.info(f"Flowbit: direct code URL, skipping resolver → {url}")
                result["direct_url"] = url
                result["filename_hint"] = unquote(Path(parsed.path).name)
                return result
