...
def smart_resolve(url: str) -> dict:
    ...
    if _url_ext in _DIRECT_EXTS:
        ...
        for _ph in _probe_headers:
            try:
                _method = "get" if ("flowbit" in host and "herokuapp.com" in host) else "head"
                _pr = getattr(requests, _method)(url, allow_redirects=True, timeout=15, headers=_ph, stream=True)
                _ct = _pr.headers.get("Content-Type", "").lower()
                _cd = _pr.headers.get("Content-Disposition", "")
                if hasattr(_pr, 'close'):
                    _pr.close()
